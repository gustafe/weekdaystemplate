...
die "enter a valid year (1900 and onwards)" unless ($year =~ m/\d{4}/ and $year >= 1900);
my %data;
for my $mon (1..12) {
    # create a Time::Piece object for the first of the current month (guaranteed to exist)
    my $first_day = Time::Piece->strptime(sprintf("%04d-%02d-%02d %02d:%02d", $year, $mon, 1, 12,0), "%Y-%m-%d %H:%M" );
	...
	push @{$data{sprintf("%04dW%02d",$day->strftime("%G"),$day->week)}}, join(' ',$day->date ,$day->day, ' ');
    }
